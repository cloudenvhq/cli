...
then
	...
	status=`curl -s --data-urlencode "slug=$slug" --data-urlencode "name=$name" --data-urlencode "version=$version" --data-urlencode "lang=cli" --data-urlencode "account=$account" -H "Authorization: Bearer $bearer" "$BASE_URL/api/v1/apps"`
	if [ "$status" == 401 ]
	then
		echo
