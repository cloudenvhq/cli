...
then
	...
	then
		...
		curl -s -H "Authorization: Bearer $bearer" "https://app.cloudenv.com/api/v1/apps/update?name=$name&keysha=$keysha" > /tmp/cloudenv-app
		echo
	...
	curl -s -H "Authorization: Bearer $bearer" "https://app.cloudenv.com/api/v1/apps/get?name=$name" > /tmp/cloudenv-app
